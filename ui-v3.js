...
const sources = [
 ['MOVIL_4G','MÓVIL 4G','◉','Compromisos móviles 4G de AWS-A, AWS-B y 2.3 GHz.'],
 ['COBERTURA_MOVIL','COBERTURA MÓVIL','▥','Compromisos de cobertura móvil por localidad o distrito.'],
 ['ACCESO_INTERNET','ACCESO INTERNET','⌁','Entidades beneficiarias, accesos y velocidades comprometidas.'],
 ['3.5 GHz-COI','3.5 GHz-COI','◉','Compromisos de cobertura de localidades asociados a la banda de 3.5 GHz.'],
 ['3.5 GHz-Km','3.5 GHz-Km','▰','Compromisos viales y sus segmentos. La unidad de cobertura es el kilómetro.'],
 ['PLANTA REPORTADA_C4','PLANTA REPORTADA C4','▤','Reportes de atención y ejecución: no son nuevos compromisos contractuales.']
];
const dims = [
...
function tabs() {
 $('projectTabs').innerHTML=(S.view==='operator'?[['ALL','TODOS LOS PROYECTOS','▦']]:[]).concat(sources).map(([id,title,icon])=>`<button class="project-tab ${S.source===id?'active':''}" data-source="${esc(id)}" aria-current="${S.source===id?'page':'false'}"><span aria-hidden="true">${icon}</span>${title}</button>`).join('');
...
function renderSummary(data) {
 ...
 if(coi)cards([['Localidades vigentes',k.localidades,'Según la información cargada','◉'],['Reemplazadas',k.localidades_reemplazadas,'Originales conservadas en el histórico','↻'],['Nuevas por reemplazo',k.nuevas_por_reemplazo,'No son obligaciones adicionales','＋'],['Sin evidencia',k.sin_evidencia_ejecucion,'No implica incumplimiento','○'],['Población-compromiso',k.poblacion_compromiso,'No representa personas únicas','♙']]);
 else if(vial)cards([['Kilómetros comprometidos',k.kilometros_comprometidos,'Incluye segmentos preservados','▰',2],['Compromisos viales',k.compromisos_vigentes,'Registros padre vigentes','◉'],['Contratos',k.contratos,'Presentes en la selección','▤'],['Con evidencia',k.con_evidencia_ejecutada,pct,'✓'],['Sin evidencia',k.sin_evidencia_ejecucion,'No implica incumplimiento','○']]);
 else cards([['Compromisos vigentes',k.compromisos_vigentes,'Excluye localidades reemplazadas','◉'],['Con evidencia',k.con_evidencia_ejecutada,pct,'✓'],['Sin evidencia',k.sin_evidencia_ejecucion,'No implica incumplimiento','○'],[internet?'Entidades':'Localidades',internet?k.entidades:k.localidades,'Identificadores únicos vigentes','♙'],[internet?'Accesos comprometidos':'Kilómetros comprometidos',internet?k.accesos_comprometidos:k.kilometros_comprometidos,internet?'Suma de accesos, no entidades':'No se suman con localidades','▰',internet?0:2]]);
...
function renderC4(data) {
 const k=data.kpi;cards([['Registros C4',k.registros,'Filas fuente, incluidas repeticiones','▤'],['Grupos únicos',k.grupos_unicos,'Agrupaciones exactas de la fuente','◉'],['Entidades',k.entidades,'Identificadas en la selección','♙'],['Atendidos reportados',k.atendidos,'Filas con estado de atención','✓'],['Sin vínculo a compromiso',k.sin_vinculo,'No se asignan por suposición','○']]);
...
function renderRows(rows) {
 ...
 $('tableBody').innerHTML=S.rows.length?S.rows.map(r=>{
  ...
  return `<tr><td><input type="checkbox" data-row="${esc(rowKey(r))}" aria-label="Seleccionar registro ${esc(rowKey(r))}"></td>${cells.map(v=>`<td title="${esc(v??'Sin dato')}">${esc(v??'Sin dato')}</td>`).join('')}<td><button class="btn" data-record="${esc(rowKey(r))}">Ver todo</button></td></tr>`;
...
const openLogin=()=>{$('loginError').textContent='';$('remember').checked=remember;$('loginDialog').showModal();};
...
$('loginDialog').addEventListener('close',()=>{$('password').value='';});
$('loginForm').onsubmit=async e=>{
 ...
 }catch(error){$('loginError').textContent='No se pudo iniciar sesión. Comprueba que esta cuenta exista en Supabase Auth y que sus credenciales sean correctas.';}
